...
## 目录
* [1. 代码的格式化](#1-代码的格式化)
	* [1-1. 指针符号 "*" 靠近变量名字](#1-1-指针符号--靠近变量名字)
	* [1-2. 只用空格缩进，1个TAB = 4个空格字符](#1-2-只用空格缩进1个tab--4个空格字符)
	* [1-3. 代码每行长度最多不超过100个字符](#1-3-代码每行长度最多不超过100个字符)
	* [1-4. 尽量将单个方法的实现代码控制在30行内](#1-4-尽量将单个方法的实现代码控制在30行内)
	* [1-5. 尽量将单个实现文件的代码行数控制在500～600行内](#1-5-尽量将单个实现文件的代码行数控制在500600行内)
	* [1-6. 方法的声明和定义](#1-6-方法的声明和定义)
	* [1-7. 方法的调用](#1-7-方法的调用)
	* [1-8. 协议声明或定义中，类型标识符、协议名称、尖括号之间不留空格](#1-8-协议声明或定义中类型标识符协议名称尖括号之间不留空格)
	* [1-9. Blocks](#1-9-blocks)
	* [1-10. 花括号的位置](#1-10-花括号的位置)
	* [1-11. 常量定义](#1-11-常量定义)
* [2. 命名规范](#2-命名规范)
	* [2-1. 文件名应该要能反映出它包含的类的名称](#2-1-文件名应该要能反映出它包含的类的名称) 
	* [2-2. Category的文件名要包含它所扩展的那个类的名称](#2-2-category的文件名要包含它所扩展的那个类的名称)	
	* [2-3. 类名、类别名字及协议名字，都采用大驼峰式命名规则。](#2-3-类名类别名字及协议名字都采用大驼峰式命名规则)
	* [2-4. 在面向特定应用的代码中，类名尽量避免使用前缀，每个类都使用相同的前缀会影响可读性。](#2-4-在面向特定应用的代码中类名尽量避免使用前缀每个类都使用相同的前缀会影响可读性)
	* [2-5. 在面向多应用的代码中，类名推荐使用前缀。](#2-5-在面向多应用的代码中类名推荐使用前缀)
	* [2-6. 类别名字](#2-6-类别名字)
	* [2-7. 方法名](#2-7-方法名)
	* [2-8. 函数名](#2-8-函数名)
	* [2-9. 属性名和变量名](#2-9-属性名和变量名)
	* [2-10. 通知名字和异常名字](#2-10-通知名字和异常名字)
* [3. 注释](#3-注释)
	* [3-1. 注释采用Javadoc风格规范](#3-1-注释采用javadoc风格规范) 
* [4. Cocoa和Objective-C特有规则](#4-cocoa和objective-c特有规则)
	* [4-1. 实例变量](#4-1-实例变量) 
	* [4-2. 注释并且清楚的指出哪一个初始化方法是你的指定初始化方法（Designated Initializer）](#4-2-注释并且清楚的指出哪一个初始化方法是你的指定初始化方法designated-initializer)
	* [4-3. 重载指定初始化方法](#4-3-重载指定初始化方法)
	* [4-4. 重载时，将NSObject相关的一些需要重载的方法放到@implementation的顶部](#4-4-重载时将nsobject相关的一些需要重载的方法放到implementation的顶部)
	* [4-5. 在初始化方法中，不要将实例变量初始化为0或nil。这是多余的](#4-5-在初始化方法中不要将实例变量初始化为0或nil这是多余的)
	* [4-6. 要避免时用 +new 方法](#4-6-要避免时用-new-方法)
	* [4-7. 保持公有API的简洁性](#4-7-保持公有api的简洁性)
	* [4-8. 使用#import引入Objective-C和Objective-C++头文件，使用#include引入C和C++头文件](#4-8-使用import引入objective-c和objective-c头文件使用include引入c和c头文件)
	* [4-9. import根框架，而非单个文件](#4-9-import根框架而非单个文件)
	* [4-10. Dealloc的顺序要与变量声明的顺序相同。如果dealloc中调用其他方法来release变量，要注释清楚这个方法release了哪些变量。](#4-10-dealloc的顺序要与变量声明的顺序相同如果dealloc中调用其他方法来release变量要注释清楚这个方法release了哪些变量)
	* [4-11. NSString类型的property优先选择copy。尽量避免对NSString使用retain](#4-11-nsstring类型的property优先选择copy尽量避免对nsstring使用retain)
	* [4-12. 避免抛出异常](#4-12-避免抛出异常)
	* [4-13. 仅在有业务逻辑需求时检查nil，而非为了防止崩溃](#4-13-仅在有业务逻辑需求时检查nil而非为了防止崩溃)
	* [4-14. Objective-C中，只允许使用BOOL。禁止将int直接转换为BOOL。将整形转换成BOOL时，应当使用三元运算符返回YES或NO，或者使用位运算符(&&, || , !)。](#4-14-objective-c中只允许使用bool禁止将int直接转换为bool将整形转换成bool时应当使用三元运算符返回yes或no或者使用位运算符---)
	* [4-15. 禁止将BOOL直接和YES比较](#4-15-禁止将bool直接和yes比较)
	* [4-16. 多层嵌套的if语句，编码时优先考虑条件不成立可以立即跳出的情况](##4-16-多层嵌套的if语句编码时优先考虑条件不成立可以立即跳出的情况)
...
#### 2-7-1. 方法名和参数名都采用小驼峰式命名规则。

	如：- (BOOL)isFileExistedAtPath:(NSString *)filePath;

#### 2-7-2. 方法名和参数名应该尽量读起来像一句话。具体参见[苹果的方法名命名规范](https://developer.apple.com/library/mac/#documentation/Cocoa/Conceptual/CodingGuidelines/Articles/NamingMethods.html)
...
#### 2-7-6. 类私有方法以下划线开头。
    ...
    根据苹果的建议，说是这种做法可能覆盖掉官方的私有方法。但是目前还没有遇到过覆盖掉官方私有方法的情况。因此，此条不做强制要求。
   
### 2-8. 函数名

#### 2-8-1. 函数名采用大驼峰式命名方式，参数名采用小驼峰式命名方式。

#### 2-8-2. 如果函数和某个特定类相关，那么函数名前缀要和类的前缀一样。

    
    
### 2-9. 属性名和变量名

#### 2-9-1. 属性名和变量名都采用小驼峰式命名规则。

#### 2-9-2. 实例变量名以下划线开头

    如：_myInstanceVariable
    
#### 2-9-3. 禁止使用匈牙利标记法来标记变量。变量名应该尽量清楚的描述它的用途。这样可以使别人立即明白代码的意思，不要担心这样会导致代码过长。
